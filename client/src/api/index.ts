...
export const anniversaryApi = {
  create: (data: { title: string; date: string; type: string; partnerName: string; description?: string }) =>
    api.post('/anniversaries', data),
  getById: (id: string) => api.get(`/anniversaries/${id}`),
  update: (id: string, data: Partial<{ title: string; date: string; type: string; partnerName: string }>) =>
